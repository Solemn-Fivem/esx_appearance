<script>
  ...
  />
  <div class="range">
    <div class="range__wrapper" tabindex="0" onkeydown={onKeyPress} bind:this={element} role="slider" aria-valuemin={min} aria-valuemax={max} aria-valuenow={value} {id} onmousedown={onTrackEvent} ontouchstart={onTrackEvent}>
      <div class="range__track" bind:this={container}>
        <div class="range__track--highlighted" bind:this={progressBar}></div>
        <div class="range__thumb" bind:this={thumb} ontouchstart={onDragStart} onmousedown={onDragStart} onmouseover={() => (thumbHover = true)} onmouseout={() => (thumbHover = false)}>
            <div></div>
        </div>
      </div>
    </div>
  ...
  <style>
    ...
    .range__track--highlighted {
      ...
      background: var(
        --track-highlight-bg,
        linear-gradient(90deg, #35C091, #0B796C)
      );
      width: 0;
      height: 6px;
      position: absolute;
    ...
    .range__thumb {
      ...
      width: 34px;
      height: 34px;
      background: linear-gradient(180deg, #35C091, #0B796C);
      cursor: pointer;
      border-radius: 999px;
      transform: translate(0, -50%);
      transition: box-shadow 100ms;
      user-select: none;
      box-shadow: 0 4px 0 0 #03362E25;
      top: 50%;
